def init(self,domain=None,overlays=[],docker_registry=None,app_domains = None):
  ...
  self.ca = certificate("ca-shalm",is_ca=True,domains=["cf-root-ca.local"])
  ...
  # Not really required
  self.workloads_certificate = certificate("workloads-certificate-shalm",signer=self.ca,domains=["*.apps.test.local"])
  self.internal_certificate = certificate("internal-certificate-shalm",signer=self.ca,domains=["*.cf-system.svc.cluster.local"])
...
def values(self):
  return inject("templates/values.yaml",self=self)
...
def apply(self,k8s):
  ...
  k8s.tool = "kapp"
  self.__apply(k8s)
  k8s.rollout_status("deployment","cf-api-server",namespace='cf-system')
